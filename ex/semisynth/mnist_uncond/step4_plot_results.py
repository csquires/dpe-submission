"""step4 plotting for mnist_uncond.
...
def main():
    p = argparse.ArgumentParser(description='plot mnist_uncond eldr estimation results')
    ...
    for metric, (ylabel, yscale) in METRICS.items():
        ...
        plot_family_boxplot(
            ...
            out_dir=figures_dir, prefix=f'mnist_uncond_{metric}', yscale=yscale,
        )

    print(f'mnist_uncond step4: family box plots for {list(METRICS)} saved to {figures_dir}')
