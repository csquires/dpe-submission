"""assemble per-slice winners.yaml for mnist peak campaign.

reads best_hp.json produced by aggregate_holdout for every
(method, slice) pair in ex.utils.hpo.optuna.configs.mnist_peak_avi,
emits schema A yaml with `per_bucket` keyed by the step2 adapter's
bucket_for_cell string ("alpha_idx_{a}").

output:
    scratch/gold_winners/winners.mnist.perslice.yaml
    (writes ONLY this file; the june winners.mnist.yaml stays untouched
    if present)

usage:
    python -m ex.semisynth.mnist.assemble_winners_perslice [--allow-missing]
...
CFG_MODULE = "ex.utils.hpo.optuna.configs.mnist_peak_avi"
OUT_PATH = Path("scratch/gold_winners/winners.mnist.perslice.yaml")


def slice_to_bucket(slice_val: int) -> str:
    """map mnist slice (alpha_idx int) -> bucket key used by adapter."""
    return f"alpha_idx_{slice_val}"
