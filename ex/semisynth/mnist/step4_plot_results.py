...
def main():
    p = argparse.ArgumentParser(description='plot mnist eldr estimation results')
    ...
    for metric, (ylabel, yscale) in METRICS.items():
        ...
        plot_family_boxplot(
            ...
            out_dir=figures_dir, prefix=f'mnist_{metric}', yscale=yscale,
        )

    print(f'mnist step4: family box plots for {list(METRICS)} saved to {figures_dir}')
