"""thin driver for mnist step3: compute per-cell eldr_abs_err, mae_train, regret.
...
if __name__ == '__main__':
    ...
    if not os.path.exists(gathered_h5):
        gathered_h5 = 'ex/semisynth/mnist/raw_results/results_all_cells.h5'
    if not os.path.exists(gathered_h5):
    ...
    # mnist est was evaluated on the 40k held-out test set (samples_test), so both
    # the pointwise mae and the eldr scalar use samples_test_true_ldrs (est-aligned).
    methods = compute_metrics(gathered_h5, cell_data_paths, out_h5=summary_h5,
                              seed=config['seed'],
                              mae_gt_key='samples_test_true_ldrs',
                              eldr_gt_key='samples_test_true_ldrs')

    # print summary table
    print("MNIST ELDR Step3: Per-Cell Metrics")
