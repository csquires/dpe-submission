...
def gen_trial_config(method: str, kl_idx: int, trial_id: int) -> dict:
    """sample one trial config and tag it with kl_idx."""
    ...
    for method in METHOD_SPECS:
        for kl_idx in range(num_kls):
            ...
            for trial_id in range(num_trials):
                cfg = gen_trial_config(method, kl_idx, trial_id)
                path = os.path.join(out_dir, f"trial_{trial_id}.json")
                with open(path, "w") as f:
                    json.dump(cfg, f, indent=2)
                total += 1

    print(f"Generated {total} configs across {len(METHOD_SPECS)} methods x {num_kls} KLs x {num_trials} trials")
