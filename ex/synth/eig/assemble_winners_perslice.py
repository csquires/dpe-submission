"""assemble per-slice winners.yaml for eig peak campaign.

reads best_hp.json produced by aggregate_holdout for every
(method, slice) pair in ex.utils.hpo.optuna.configs.eig_peak_avi,
emits schema A yaml with `per_bucket` keyed by the step2 adapter's
bucket_for_cell string ("pct_{p}_prior_{q}").

output:
    scratch/gold_winners/winners.eig.perslice.yaml
    (writes ONLY this file; the june winners.eig.yaml is left untouched)

usage:
    python -m ex.synth.eig.assemble_winners_perslice [--allow-missing]
...
CFG_MODULE = "ex.utils.hpo.optuna.configs.eig_peak_avi"
OUT_PATH = Path("scratch/gold_winners/winners.eig.perslice.yaml")


def slice_to_bucket(slice_tuple: tuple[int, int]) -> str:
    """map eig slice (pct_idx, prior_idx) -> bucket key used by adapter."""
    p, q = slice_tuple
