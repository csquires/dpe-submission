"""assemble per-slice winners.yaml for occupancy peak campaign.

reads best_hp.json produced by aggregate_holdout for every
(method, slice) pair in ex.utils.hpo.optuna.configs.occupancy_peak_avi,
emits schema A yaml with `per_bucket` keyed by the step2 adapter's
bucket_for_cell string ("k1_idx_{k1}").

output:
    scratch/gold_winners/winners.occupancy.perslice.yaml
    (writes ONLY this file; the june winners.occupancy.yaml is left untouched)

usage:
    python -m ex.synth.occupancy.assemble_winners_perslice [--allow-missing]
...
slice-to-bucket:  occupancy slices are (k1_idx, beta_idx) tuples where all
current slices share beta_idx=0.  the step2 adapter's bucket_for_cell
collapses to `k1_idx_{k1}` (adapter line 132-134), so slice_to_bucket drops
beta_idx and returns the same shape.
"""
...
CFG_MODULE = "ex.utils.hpo.optuna.configs.occupancy_peak_avi"
OUT_PATH = Path("scratch/gold_winners/winners.occupancy.perslice.yaml")


def slice_to_bucket(slice_tuple: tuple[int, int]) -> str:
    """map occupancy slice (k1_idx, beta_idx) -> bucket key used by adapter.

    the adapter's bucket_for_cell drops beta_idx and returns f"k1_idx_{k1}"
    (occupancy step2_adapter.py:132-134). we mirror that here.
    """
