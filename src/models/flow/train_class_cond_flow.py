"""pure function for training velocity matching models via class-conditional flow matching."""
...
def train_class_cond_flow(
    model: nn.Module,
    codes: Tensor,              # [N, D] raw tensor of training data
    labels: Tensor,             # [N] long class labels in [0, num_classes)
    ...
    """train velocity matching model via class-conditional flow matching with ema regularization.
    ...
    train loop:
      1. sample batch of data and label indices
      ...
      6. predict velocity: v_pred = model(x_t, t, y)
      7. loss = mse(v_pred, v)
      8. optimize
    ...
    args:
      model: nn.Module with forward(x_t: [B, D], t: [B,], labels: [B,]) -> [B, D]
      codes: [N, D] training data
      labels: [N] long class labels in [0, num_classes)
    ...
    # phase 2b: validation
    assert labels.dtype == torch.long, f"labels dtype must be torch.long, got {labels.dtype}"
    assert labels.shape == (N,), f"labels shape must be ({N},), got {labels.shape}"
    assert codes.shape[0] == N, f"codes and labels must have same first dim, got {codes.shape[0]} vs {N}"
    ...
    for step in range(total_steps):
        ...
        x1 = codes[idx]  # [batch_size, D]
        y = labels[idx]  # [batch_size]
        ...
        # 5f: model prediction (class-conditional)
        v_pred = model(x_t, t, y)  # [batch_size, D]
