"""
Step 2b: Merge parallel DRE results for Sample Complexity Experiment
...
with h5py.File(f'{RAW_RESULTS_DIR}/results_task_0.h5', 'r') as f:
    ...
    n_nsamples_train = f.attrs['n_nsamples_train']
    nsamples_test = f.attrs['nsamples_test']
    alg_names = json.loads(f.attrs['algorithm_names'])

print(f"Merging {num_tasks} task files")
print(f"Total datasets: {nrows_total}")
print(f"Number of training sample sizes: {n_nsamples_train}")
print(f"Test samples per evaluation: {nsamples_test}")
...
output_file = f'{RAW_RESULTS_DIR}/results.h5'

with h5py.File(output_file, 'w') as out_f:
    # Merge each algorithm
    for alg_name in alg_names:
        dataset_name = f'est_ldrs_arr_{alg_name}'
        # Shape: (nrows_total, n_nsamples_train, nsamples_test)
        full_arr = np.zeros((nrows_total, n_nsamples_train, nsamples_test))
