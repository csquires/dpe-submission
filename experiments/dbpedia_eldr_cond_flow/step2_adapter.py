"""step2_runner adapter for dbpedia_eldr_cond_flow.

mirrors mnist_eldr_cond_flow structure: cell axis = flat (alpha_idx, pair_idx),
bucket = alpha_idx_<n>, per-cell <data_dir>/alpha_<a>_pair_<p>.h5.

note: my v2 winners file is named winners.dbpedia_cond_flow.uniform_200broad.yaml
(not dbpedia_eldr_cond_flow). pass that path explicitly to dispatch --winners.

input_dim is config['pca_dim'] (64) — not latent_dim.
"""
...
def _input_dim(config: dict) -> int:
    # dbpedia uses pca_dim (post SBERT->PCA reduction), not latent_dim
    return config.get("pca_dim", config.get("latent_dim", 64))
...
def fit_and_eval(method: str, hp: dict, cell_idx: int, config: dict,
    ...
    if method not in SEARCH_SPACES:
        raise KeyError(f"method {method!r} not in dbpedia_eldr_cond_flow SEARCH_SPACES")
...
def gather_output_path(config: dict) -> str:
    return os.path.join(config.get("raw_results_dir",
        "experiments/dbpedia_eldr_cond_flow/raw_results"), "results_all_cells.h5")
