...
with h5py.File(output_file, 'w') as out_f:
    # Merge true_eigs_arr
    true_eigs_arr = np.zeros(nrows_total, dtype=np.float32)
    for task_id in range(num_tasks):
        with h5py.File(f'{RAW_RESULTS_DIR}/results_task_{task_id}.h5', 'r') as f:
            start_idx = f.attrs['start_idx']
            end_idx = f.attrs['end_idx']
            true_eigs_arr[start_idx:end_idx] = f['true_eigs_arr'][:]
    out_f.create_dataset('true_eigs_arr', data=true_eigs_arr)
    print(f"Merged true_eigs_arr: shape {true_eigs_arr.shape}")

    # Merge each algorithm
    for alg_name in alg_names:
        dataset_name = f'est_eigs_arr_{alg_name}'
        full_arr = np.zeros(nrows_total, dtype=np.float32)
