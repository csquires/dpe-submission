...
with h5py.File(raw_results_filename, 'r') as results_file:
    ...
    est_keys = [key for key in results_file.keys() if key.startswith('est_eigs_arr_vertex_')]
    est_eigs_by_vertex = {key.replace('est_eigs_arr_', ''): results_file[key][:] for key in est_keys}
...
for vertex_name, est_eigs in est_eigs_by_vertex.items():
    ...
    mae_by_beta[vertex_name] = np.array(beta_maes, dtype=np.float32)
...
with h5py.File(processed_results_filename, 'w') as out_file:
    ...
    for vertex_name, maes in mae_by_beta.items():
        out_file.create_dataset(f'mae_by_beta_{vertex_name}', data=maes)
