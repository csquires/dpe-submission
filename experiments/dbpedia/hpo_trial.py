...
from dbpedia_eldr), fits, predicts ldr, computes mae. delegates to
run_trial() for seeding, aggregation, and atomic json write. differs from
mnist_eldr only in the config.yaml path and SEARCH_SPACES import.
"""
...
def main():
    ...
    with open("experiments/dbpedia/config.yaml", "r") as f:
