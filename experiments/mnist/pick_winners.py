"""
pick_winners.py (MNIST ELDR): aggregate HPO trial results and select top-K
per (method, alpha) pair. reads config to infer alpha indices.
"""
...
def main():
	"""parse CLI args, read config, auto-discover methods from results_dir,
	collect top-K winners, write YAML."""
	...
	default_results_dir = os.path.expandvars(
		"/data/user_data/$USER/dpe-submission/mnist_eldr/hpo_results"
	)
	parser.add_argument(
		"--results-dir",
		type=str,
	...
	parser.add_argument(
		"--output",
		type=str,
		default="experiments/mnist/winners.yaml",
